...
file_path_hs <- "data/out/4_ibd_ibc/HS_NS/ORYX_geno_HS_NS_biallelic_chr_mdepth_miss_maf.hom"

roh_lengths_hs <- fread(file_path_hs)
length(unique(roh_lengths_hs$FID))

data_hs <- roh_lengths_hs %>%
  left_join(meta, by = c("FID" = "Sample_ID")) %>%
  filter(CHR != "NW_024070207.1")

froh_hs <- data_hs %>%
  dplyr::group_by(FID, Origin, WGS_run) %>%
  dplyr::summarise(KBAVG = mean(KB), KBSUM = sum(KB)) %>%
  mutate(FROH_HS = KBSUM/auto_length) %>%
  ungroup() %>%
  select(FID, FROH_HS)

froh <- left_join(froh, froh_hs)

ds_hs_froh <- ggplot(filter(froh, WGS_run == 1), aes(FROH, FROH_HS, col = Origin)) +
  geom_point(size = 3) +
  theme_emily() +
  scale_color_manual(values = col_palette, name = "Population") +
  ylab(expression(paste("High coverage ",italic(F["ROH"])))) +
  xlab(expression(paste("Downsampled ",italic(F["ROH"]))))
...
roh_lengths %>%
  select(FID, KB, WGS_run, Origin) %>%
  mutate(COV = "DS") %>%
  left_join(data_hs, by ="FID") %>%
  filter(WGS_run == 1) %>%
  pivot_longer(cols = c(KB, KB_HS)) %>%
  mutate(name = case_when(name == "KB" ~ "Downsampled",
                          TRUE ~ "High coverage")) %>%
  ggplot(aes(value, fill = name)) +
  geom_histogram() +
  scale_color_manual(values = col_palette, name = "") +
  theme_emily() + xlab("ROH length") + ylab("Count")
...
ggsave("figs/ROH_n_l.png", roh_n + roh_l, width = 8, height = 4)
...
len_dis <- roh_lengths %>%
  filter(CHR != "NW_024070207.1") %>% # X chr
...
col_pal <- paste0("#", (c("666666","838383","9e9e9e","c5c5c5","e5e5e5","e5e5e5")))

recent_demog_sp <- ggplot(len_dis, aes(as.factor(length_class), prop_IBD, fill = length_class)) +
  geom_half_point(side = "l", shape = 21, alpha = 0.5, stroke = 0.1, size =4,
                  transformation_params = list(height = 0, width = 1.3, seed = 1)) +
  geom_half_boxplot(side = "r", outlier.color = NA,
                    width = 0.6, lwd = 0.3, color = "black",
                    alpha = 0.8) +
  scale_fill_manual(values = col_pal, name = "Population") +
  theme_emily() +
  theme(axis.ticks.x = element_line(colour = "#cccccc", size = 0.3),
        legend.position = "none") +
        #axis.text.x = element_text(angle = 40, vjust = 1, hjust = 1)) +
...
# Alternative Ne calculation method
...
alt_ne <- roh_lengths %>%
  ...
  mutate(class = as.factor(class)) %>%
  select(-Origin, class) %>%
  group_by(FID, class, .drop = F) %>%
  dplyr::summarise(prop_IBD = sum((length_Mb / 2439) * 100)) %>%
  mutate(prop_IBD = prop_IBD / 100) %>%
  left_join(meta, by = c("FID" = "Sample_ID")) %>%
  mutate(cumu_prop_IBD = cumsum(prop_IBD)) %>%
  mutate(mean_froh = cumu_prop_IBD - lag(cumu_prop_IBD)) %>%
  mutate(mean_froh = case_when(is.na(mean_froh) ~ cumu_prop_IBD,
                               TRUE ~ mean_froh))

alt_ne <- alt_ne %>% 
  group_by(class, Origin) %>%
  summarise(mean_froh = mean(mean_froh)) %>%
  filter(!is.na(mean_froh)) %>%
...
# scp ehumble@eddie.ecdf.ed.ac.uk:/exports/cmvm/eddie/eb/groups/ogden_grp/emily/SHO_reseq_2022/data/out/5_angsd/DS_NS/sfs/*pestPG data/out/5_angsd/DS_NS/sfs/

data_path <- "data/out/5_angsd/DS_NS/sfs/"
files <- dir(data_path, pattern = "*\\PG")

# Number of 0,1,2 genotypes per individual across SNP classes

theta <- tibble(filename = files) %>%
  mutate(file_contents = map(filename,
                             ~ fread(file.path(data_path, .)))) %>%
  unnest(cols = c(file_contents)) %>%
  separate(filename, c("POP", "CHR")) %>%
  group_by(POP) %>%
  summarise(mintW = min(tW),
            maxtW = max(tW),
            meantW = mean(tW),
            mintP = min(tP),
            maxtP = max(tP),
            meantP = mean(tP),
            minTajima = min(Tajima),
            maxTajima = max(Tajima),
            meanTajima = mean(Tajima))

theta %>%
  mutate(sci = meantP/100000)
